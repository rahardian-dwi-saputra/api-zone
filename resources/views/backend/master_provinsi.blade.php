                    	...
                    	<div class="row">
                    		<div class="col-lg-12">
                    			...
                    			<div class="table-responsive">
                    				<table class="table table-striped table-bordered table-hover" id="data-provinsi">
                    					<thead>
                                    		<tr>
                                        		<th>No</th>
                                        		<th>Nama Provinsi</th>
            	...
            	<div class="modal-body">
            		...
            		<div class="form-group row">
    					<label for="nama_provinsi" class="col-sm-3 col-form-label">
    						Nama Provinsi <span style="color:red;">*</span>
    					</label>
    					<div class="col-sm-9">
...
<script>
	$(function (){
		...
		var table = $('#data-provinsi').DataTable({
        	processing: true,
            serverSide: true,
            ajax: "/dataprovinsi",
            columns: [
		...
		$('#plus-button').click(function(){
			$('#myModalLabel').text('Tambah Data Provinsi');
            $('#form-input')[0].reset();
            $('#id').val('');
		...
		$(document).on('click', 'a#edit', function(e){
			...
			$.ajax({
				url: "/dataprovinsi/"+$(this).attr('href'),
				type: 'GET',
                dataType: 'json',
                success: function(response){
                    if(response.success == true){
                        $('#myModalLabel').text('Edit Data Provinsi');
                        $('#method').val('update');
                        $('#id').val(response.data.id);
                        $('#nama_provinsi').val(response.data.nama_provinsi);
                        $('#myModal').modal('show');
                    }else{
		...
		$('#form-input').submit(function(e){
			...
			if($('#method').val() == 'insert')
				var url_link = "/dataprovinsi";
			else{
				data.push({name:"_method", value:"PUT"});
				var url_link = "/dataprovinsi/"+$('#id').val();
			}
